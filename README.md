...
Программа принимает на вход PDF документ с содержанием русского/белорусского/укаринского/английского языков, далее при помощи Paddle OCR распознает его содержимое. Содержимое документа добается на вход к одной из моделей на выбор: Phi2 (~ 3 млр. параметров) или Mistral7b (7 миллиардов параметров), после на выход выдается JSON-файл с запросами к базе данных согласно указанной пользователем схеме БД и промптом. Сочетает в себе элементы тонкой настройки и обращение к схеме БД пользователя.

В config.yaml содержит схему базы данных, необходимо указать свою и промпт для работы с получаемым текстом.
